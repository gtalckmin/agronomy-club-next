...
4. Execute the import job once with `--expect-clean-dry-run=6`; the job fails unless exactly six records are clean candidates and every skip count is zero. Execute it with `--apply` only after that dry-run gate passes. Re-run dry-run mode to establish that all six are existing profiles.
